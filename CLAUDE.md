...
## 개인정보 금지 원칙 (필수)
- 유아 **이름·진단명·약물명·보호자(학부모) 연락처 저장 금지**. 스키마에 해당 컬럼을 두지 않는다.
- 취약 유아 정보는 **숫자 집계값만** 저장.
- **AI API에 개인식별정보(PII)를 전송하지 않는다**(화이트리스트 필드만 직렬화). 교직원 연락처도 AI 입력에 절대 포함하지 않는다.
- **예외(2026-06-16 결정)**: 대응계획 공유·발송 목적에 한해 **교직원 업무 연락처**(역할별 담당자)는 수신동의(consent_*)를 함께 저장 허용. 저장 위치는 `institution_staff_contacts`(anon SELECT 정책 없음 = service_role 전용). 학부모 연락처는 1차 MVP에서 계속 저장 금지.
